...
func (jwt *JWT) RefreshToken(c *gin.Context) (string, error) {
	...
	// 5. 检查是否过了『最大允许刷新的时间』
	x := _time.TimeNowInTimezone().Add(-jwt.MaxRefresh).Unix()
...
func (jwt *JWT) IssueToken(userID string, userName string) string {
	...
	claims := JWTCustomClaims{
		...
		expireAtTime,
		jwtpkg.StandardClaims{
			NotBefore: _time.TimeNowInTimezone().Unix(), // 签名生效时间
			IssuedAt:  _time.TimeNowInTimezone().Unix(), // 首次签名时间（后续刷新 Token 不会更新）
...
func (jwt *JWT) expireAtTime() int64 {
	timenow := _time.TimeNowInTimezone()
